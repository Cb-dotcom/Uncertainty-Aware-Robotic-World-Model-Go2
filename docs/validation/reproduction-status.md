...
The picture changed substantially again in Phase 4G. The earlier frontier, "a seed-fragile offline
walker at offline-ANYmal parity", is now a *supporting* result, not the headline. The headline is the
**uncertainty-calibration finding**: broadening offline data improves prediction while flattening the
ensemble disagreement MOPO needs, replicated across two exploit traces, with the MSE-vs-disagreement
decoupling as proof it is miscalibration rather than ignorance. The mechanism is the **shared-trunk +
